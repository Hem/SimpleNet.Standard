...
using SimpleNet.Standard.Data.Utility;
...
namespace SimpleNet.Standard.Data.Mappers
{
    ...
    public abstract class PropertyMapping
    {
        ...
        protected static object ConvertNonNullableValue(object value, Type conversionType)
        {
            ...
            else if (conversionType.GetTypeInfo().IsValueType)
            {
    ...
    public class ColumnNameMapping : PropertyMapping
    {
        ...
        public override object GetPropertyValue(DbDataReader row)
        {
            ...
            {
                throw new InvalidOperationException(
                    string.Format(CultureInfo.CurrentCulture, "Column {0} not found while mapping.", ColumnName),
                    ex);
            }
            ...
            catch (InvalidCastException castException)
            {
                string exceptionMessage =
                    string.Format(
                        CultureInfo.CurrentCulture,
                        "The value from column '{0}' cannot be converted to property '{1}' of type '{2}'.",
                        ColumnName,
                        Property.Name,
